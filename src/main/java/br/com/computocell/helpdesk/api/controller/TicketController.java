package br.com.computocell.helpdesk.api.controller;
...
import br.com.computocell.helpdesk.api.dto.Summary;
import br.com.computocell.helpdesk.api.entity.User;
import br.com.computocell.helpdesk.api.response.Response;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> createdOrUpdate(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			...
			Ticket ticketPersisted = (Ticket) ticketService.createdOrUpdate(ticket);
	...
	/**
	 * Retorna o usuario a partir do request Pega o token e e o email e utilizando o
	 * jwtTokenUtil retorna o usuario responsável
	 * 
	 * @param request
	 * @return
	 */
	...
	/**
	 * Gera um numero aleatorio para ticket
	 * 
	 * @return
	 */
	...
	/**
	 * Valida a atualização dos tickets
	 * 
	 * @param ticket
	 * @param result
	 */
	...
	public ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, @PathVariable("page") int page,
		...
		if (number > 0) {
			...
			if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				...
				tickets = ticketService.findByParametersAncCurrentUser(page, count, title, status, priority,
						userRequest.getId());
			}
		}
		response.setData(tickets);
	...
	public ResponseEntity<Response<Ticket>> changeStatus(@PathVariable("id") String id,
		...
		try {
			...
			Ticket ticketPersisted = (Ticket) ticketService.createdOrUpdate(ticketCurrent);
	...
	public ResponseEntity<Response<Summary>> findSummary() {
		...
		Integer ammountNew = 0;
		Integer ammountResolved = 0;
		Integer ammountApproved = 0;
		Integer ammountDissaproved = 0;
		Integer ammountAssigned = 0;
		Integer ammountClosed = 0;
		...
		summary.setAmmountNew(ammountNew);
		summary.setAmmountResolved(ammountResolved);
		summary.setAmmountApproved(ammountApproved);
		summary.setAmmountDissaproved(ammountDissaproved);
		summary.setAmmountAssigned(ammountAssigned);
		summary.setAmmountClosed(ammountClosed);
